...
testOnRandHM :: PropertyT IO HMTest -> Property
testOnRandHM propHmtest = property $ do
  (kv : kvs) <- forAll randKeyVals
...
-- | Key generator
randKey :: Gen Int
randKey = Gen.int $ Range.linearFrom 0 (-900) 900
...
-- | Random pairs
randKeyVals :: Gen [(Int, String)]
randKeyVals = do
  randSize <- Gen.int $ Range.constantFrom 600 400 800
  let randSizeGen = Range.singleton randSize
  keys <- Gen.list randSizeGen randKey
  vals <- Gen.list randSizeGen randVal
...
lookupSing1 = property $ do
  key <- forAll randKey
  val <- forAll randVal
  let test = testKVPairExists (key, val)
  assert $ unUnrestricted Linear.$ HashMap.singleton (key, val) test

lookupSing2 :: Property
lookupSing2 = property $ do
  key <- forAll randKey
  val <- forAll randVal
  key' <- forAll $ Gen.filter (/= key) randKey
  let test = testKeyMissing key'
  assert $ unUnrestricted Linear.$ HashMap.singleton (key, val) test

lookupInsert1 :: Property
lookupInsert1 = testOnRandHM $ do
  key <- forAll randKey
  val <- forAll randVal
  let insertKV = insertPair (key, val)
  let checkKV = testKVPairExists (key, val)
  return (checkKV Linear.. insertKV)

lookupInsert2 :: Property
lookupInsert2 = testOnRandHM $ do
  key <- forAll randKey
  key' <- forAll $ Gen.filter (/= key) randKey
  val <- forAll randVal
  let insertKV = insertPair (key, val)
  return (testLookupUnchanged insertKV key')

lookupDelete1 :: Property
lookupDelete1 = testOnRandHM $ do
  key <- forAll randKey
  let deleteK = deleteKey key
  let checkNoKey = testKeyMissing key
  return (checkNoKey Linear.. deleteK)

lookupDelete2 :: Property
lookupDelete2 = testOnRandHM $ do
  key <- forAll randKey
  key' <- forAll $ Gen.filter (/= key) randKey
  let deleteK = deleteKey key
  return (testLookupUnchanged deleteK key')

memberInsert :: Property
memberInsert = testOnRandHM $ do
  key <- forAll randKey
  val <- forAll randVal
  let insertKV = insertPair (key, val)
  let isMemberK = testKeyMember key
  return (isMemberK Linear.. insertKV)

memberDelete :: Property
memberDelete = testOnRandHM $ do
  key <- forAll randKey
  val <- forAll randVal
  let pair = (key, val)
  let deleteK = deleteKey key
  let insertKV = insertPair pair
  let checkNotMember = testKeyNotMember key
  return (checkNotMember Linear.. deleteK Linear.. insertKV)

sizeInsert :: Property
sizeInsert = testOnRandHM $ do
  key <- forAll randKey
  val <- forAll randVal
  let pair = (key, val)
  let deleteK = deleteKey key
...
deleteSize :: Property
deleteSize = testOnRandHM $ do
  key <- forAll randKey
  val <- forAll randVal
  let insertKV = insertPair (key, val)
  let checkSize = checkSizeAfterDelete key
